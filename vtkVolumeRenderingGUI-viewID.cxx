...
void vtkVolumeRenderingGUI::AddVolumeToViewers()
{
  ...
  for (int i = 0; i < numViewer; i++)
  {
    ...
    if (slicer_viewer_widget)
    {
      ...
      if ( vspNode && vspNode->GetCurrentVolumeMapper() == 1 )
        {
        // Here the parameter node has requested the GPU mapper which can only be shown in
        // one renderwindow or it will cause a crash in opengl.  If the parameter node
        // specifies a view node id, then only add the view prop to the viewer that
        // matches that id.
        const char *preferredID = vspNode->GetPreferredViewID();
        const char *viewID = slicer_viewer_widget->GetViewNode()->GetID();
        if ( preferredID && !strcmp(preferredID, viewID) )
          {
          ...
          if ( i == 0 ) 
            {
            // node didn't specify, so add it to the first one
            slicer_viewer_widget->GetMainViewer()->AddViewProp(this->GetLogic()->GetVolumeActor() );
            }
          }
        }
...
void vtkVolumeRenderingGUI::RemoveVolumeFromViewers()
{
  ...
  {
    vtkSlicerViewerWidget *slicer_viewer_widget = this->GetApplicationGUI()->GetNthViewerWidget(i);
    slicer_viewer_widget->GetMainViewer()->GetRenderWindow()->RemoveObserver((vtkCommand*)this->GUICallbackCommand);
    if (slicer_viewer_widget)
    {
