...
void compute_derivative (const int signalSize,
                         ...
                         float*& YDeriv)
{
  const int n = signalSize;
  if (YDeriv == NULL)
    YDeriv = new float[n];
  //memset(YDeriv, '\0', n*sizeof(float));

  YDeriv[0] = (float) ((-3.0*SignalY[0] + 4.0*SignalY[1] - SignalY[2]) / 2.0);
  YDeriv[n-1] = (float) ((3.0*SignalY[n-1] - 4.0*SignalY[n-2] + SignalY[n-3]) / 2.0);
  for(int i=1; i<n-1; i++) {
...
{  
  ...
  float MaxSlope;
  bool result = compute_bolus_arrival_time (signalSize, SignalY, ArrivalTime, FirstPeak, MaxSlope);
  if (result == false) {
